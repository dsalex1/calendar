<template>
  <div v-for="date of dates" :key="date.toISO()" class="d-flex border-bottom p-2 flex-wrap">
    ...
    <div class="flex-grow-1">
      <div v-for="event of events.filter((e) => e.start.split('T')[0] == date.toFormat('yyyy-LL-dd'))" :key="event.id" class="mb-2 d-flex">
...
const dates = computed(() =>
  [...new Set(events.value.map((e) => e.start.split("T")[0]))].sort().map((d) => DateTime.fromFormat(d, "yyyy-LL-dd").startOf("day"))
);
</script>
